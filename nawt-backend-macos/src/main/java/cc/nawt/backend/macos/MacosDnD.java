 ...
 * A shared {@code NawtDragSource} class conforms to {@code NSDraggingSource}
...
final class MacosDnD {
    ...
    private static final MemorySegment PAN_ACTION_SEL = Objc.sel("nawtDnDPan:");
    ...
    private static void installPanGestureRecognizer(MemorySegment view) {
        // [[NSPanGestureRecognizer alloc] initWithTarget:DRAG_SOURCE_INSTANCE action:@selector(nawtDnDPan:)]
    ...
    @SuppressWarnings("unused") // upcalled from native via nawtDnDPan:
    ...
    private static MemorySegment buildDropSubclass(MemorySegment origCls, String origName) {
        String subName = "NawtDrop_" + origName;
    ...
    private static MemorySegment registerDragSourceClass() {
        MemorySegment cls = Objc.allocateClassPair(Objc.cls("NSObject"), "NawtDragSource");
        try {
            ...
            // -(void)nawtDnDPan:(NSPanGestureRecognizer*)recognizer → "v@:@"
