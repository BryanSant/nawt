...
public final class Toolkit {
    ...
    public static Toolkit requireLaunched() {
        ...
        if (t == null) {
            throw new IllegalStateException(
                "Nawt toolkit has not been launched. Call Toolkit.launch(...) before using widgets.");
        }
        return t;
    }
    ...
    public static void launch(String appName, Runnable appMain) {
        ...
        loop.bootstrap();
        Thread app = Thread.ofVirtual().name("nawt-app").unstarted(() -> {
            try {
                appMain.run();
            } catch (Throwable e) {
                e.printStackTrace();
    ...
    public static java.util.concurrent.CompletableFuture<String> showFileOpenDialog(
            cc.nawt.spi.FileDialogConfig config) {
    ...
    public static java.util.concurrent.CompletableFuture<String> showFileOpenDialog(String title) {
        return showFileOpenDialog(new cc.nawt.spi.FileDialogConfig(title, null, null));
    }
    ...
    public static java.util.concurrent.CompletableFuture<String> showFileSaveDialog(
        ...
        return showFileSaveDialog(new cc.nawt.spi.FileDialogConfig(title, null, defaultName));
    }
    ...
    public static java.util.concurrent.CompletableFuture<String> showFolderDialog(
            cc.nawt.spi.FolderDialogConfig config) {
    ...
    public static java.util.concurrent.CompletableFuture<String> showFolderDialog(String title) {
        return showFolderDialog(new cc.nawt.spi.FolderDialogConfig(title, null));
    }

    /**
     * Resolve the backend {@link PeerFactory}: honors the
     * {@code -Dnawt.backend=<id>} system property, otherwise picks the first
     * factory whose {@link PeerFactory#supports() supports()} returns true.
     */
    public static PeerFactory detect() {
        String override = System.getProperty("nawt.backend");
        ...
        if (factories.isEmpty()) {
            throw new IllegalStateException(
                "No nawt backend on the module path. Add a runtime dependency on "
                + "cc.nawt.backend.macos or cc.nawt.backend.gtk.");
        }
        ...
        throw new IllegalStateException(
            "No nawt backend supports this host. Available: "
            + factories.stream().map(PeerFactory::platformId).toList());
    }
}
